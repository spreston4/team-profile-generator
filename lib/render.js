...
const generateCards = () => {
    ...
    let card = getCard('manager');

    card = card.replace('[NAME]', testArr[i].name);
    // card = card.replace()
    card = card.replace('[ID]', testArr[i].id);
    card = card.replace('[EMAIL]', testArr[i].email);
    card = card.replace('[EMAIL]', testArr[i].email);
    // card = card.replace('[OFFICE]', testArr[i].officeNumber);
